...
+## [1.0.37] - 2026-09-18
+
+### Подробная идентификация накопителей и дисковых разделов
+- **Определение моделей, интерфейсов и скоростей накопителей:**
+  - Извлечение точных названий физических дисков и флешек (например, `Netac OnlyDisk`, `Seagate Expansion Desk`, `Встроенное хранилище`) через sysfs.
+  - Определение типа носителя: `USB Flash`, `HDD`, `SSD`, `NAND Flash`, `RAM / tmpfs`.
+  - Определение интерфейса и скорости подключения: `USB 3.0 • 5 Гбит/с`, `USB 2.0 • 480 Мбит/с`, `Внутренний`, `ОЗУ`.
+  - Автоматическое распознавание пользовательских меток томов и симлинков Keenetic (`/tmp/mnt/Opkg`, `/tmp/mnt/Seagate Expansion Drive`).
+  - Фильтрация служебных каталогов tmpfs внутри `/tmp/mnt/` (например, `ChatData`).
+- **Обновленный дизайн карточек дисков:**
+  - Компактные цветные бейджи интерфейса, скорости и типа накопителя.
+  - Отображение точки монтирования, файловой системы, устройства (`/dev/sda1`) и метки тома.
+
 ## [1.0.36] - 2026-09-18

### Устранение зависания показателей CPU и системы в 0 (включая KeenDNS)
- **Гибридный режим телеметрии (WebSocket + HTTP Polling):**
  - Реализован автоматический переход на HTTP-опрос `/api/system/snapshot` (каждые 3 сек), если WebSocket не может подключиться, сбрасывается или блокируется облачным прокси KeenDNS Cloud.
  - Бесшовное автоматическое возвращение на WebSocket-поток в реальном времени при восстановлении прямого соединения.
  - Наглядная индикация режима в шапке панели: `Live: WS` (зеленый индикатор) и `Live: HTTP` (бирюзовый индикатор для KeenDNS/Proxy).
- **Мгновенная первичная загрузка карточек метрик:**
  - При открытии веб-панели сразу отправляется первичный HTTP-запрос метрик системы, исключая пустое зависание в `0.0%` и `0 / 0 MB` на карточках при холодном старте.
  - Синхронизация кнопки «Обновить»: ручное обновление актуализирует как таблицу процессов, так и показатели CPU, RAM, дисков и сети.
- **Калибровка «холодного старта» CPU на бэкенде:**
  - Анализатор CPU выполняет быстрый 80-мс срез при отсутствии базовых тиков процессора, гарантируя реальные проценты загрузки CPU уже на первом замере.
  - Первый снимок WebSocket отправляется клиенту моментально после рукопожатия без 2-секундного ожидания тикера.

## [1.0.35] - 2026-09-17

### Тестовый релиз обновления (Update Verification Release)
- Проверка механизма бесшовного самообновления (Self-Update) без задержек и залипания кэша.
- Мгновенная синхронизация версий в интерфейсе веб-панели.

## [1.0.34] - 2026-09-17

### Устранение кэширования и моментальное отображение новой версии
- **Анти-кэширование API и HTML:**
  - Добавлены строгие заголовки `Cache-Control: no-store, no-cache, must-revalidate, max-age=0` для всех ответов API (`/api/status`, `/api/version/check` и др.) и главной страницы `/`.
  - Устранено залипание старой версии в браузере при перезагрузке страницы.
- **Умный опрос статуса после обновления (Poll Status Sync):**
  - Процесс обновления теперь гарантированно дожидается фактического запуска именно новой версии демона перед завершением и перезагрузкой веб-страницы.
  - При перезагрузке страницы используется параметр сброса кэша браузера `?_v=...`.

## [1.0.33] - 2026-09-17

### Безотказное управление службами Entware (/opt/etc/init.d/)
- **Устранение ошибок `permission denied`:**
  - Добавлен автоматический запуск скриптов служб через оболочку `/bin/sh <script> <action>`, если у файла отсутствует бит исполнения (`+x`) или раздел `/opt` смонтирован с ограничением `noexec`.
  - Автоматическое восстановление прав доступа (`chmod 0755`) при повторных ошибках доступа.
- **Гарантированная остановка служб:**
  - При сбое выполнения скрипта `stop` служба принудительно завершается сигналом ядра (`killall -9 <procName>`).
- **Защищенный опрос статуса:**
  - Проверки `check` и `status` переведены на защищенный вызов `runInitScript`.

## [1.0.32] - 2026-09-15

### Кэширование процессов и умное автообновление (Top / Processes)
- **Серверный микро-кэш (1.5 сек):** запросы к `/api/system/processes` троттлятся в памяти для защиты слабого CPU роутера и обеспечения стабильной базы для точного расчета CPU%.
- **Кэширование аргументов процессов (`cmdline`):** неизменяемые аргументы процессов считываются с диска только при первом появлении PID, исключая ~100 системных вызовов чтения на каждый такт.
- **Инвалидация кэша при Kill:** мгновенный сброс кэша при отправке сигнала процессу для немедленного обновления таблицы.
- **Интеллектуальное автообновление в веб-панели:**
  - Селектор интервала: `Авто: [ Выкл | 2 сек | 3 сек | 5 сек ]` с сохранением выбора в `localStorage`.
  - Зеленый пульсирующий маячок активности при каждом обновлении списка процессов.
  - **Zero Background Load:** таймер автообновления работает только при открытой вкладке «Система» и активном окне браузера; при переключении на другие вкладки («Файлы», «Терминал») или сворачивании браузера опрос полностью засыпает (0% нагрузки на роутер).
  - Сохранение строки поиска, сортировки колонок и позиции скролла при автообновлении.

## [1.0.31] - 2026-09-12

### Комплексная оптимизация оперативной памяти (Low-RAM Tuning)
- **Ядро Go Runtime и управление кучей**:
  - Установлены значения по умолчанию `GOGC=30` и мягкий лимит памяти `GOMEMLIMIT=24MiB` в приложении и в службе `/opt/etc/init.d/S99smart-utils`.
  - Фоновый сборщик памяти периодически вызывает `debug.FreeOSMemory()`, возвращая неиспользуемые страницы кучи ядру Linux роутера.
- **Потоковый разбор пакетов OPKG без аллокаций**:
  - Полный переход на потоковое сканирование списков пакетов поверх `gzip.Reader` без вычитывания многомегабайтных файлов в память и без `strings.Split`.
  - Пиковые аллокации сокращены с 25–40 МБ до ~64 КБ.
  - Добавлен метод `GetCachedPackage` для моментального опроса статуса без повторного сканирования репозиториев и автоочистка кэша при простое.
- **Безопасность загрузки файлов (Upload RAM Protection)**:
  - Буфер памяти `ParseMultipartForm` ограничен 4 МБ вместо 128–256 МБ. Большие файлы и бэкапы безопасно стримятся на диск, предотвращая OOM на роутерах с 128–256 МБ RAM.
  - Добавлена гарантированная очистка временных структур через `defer r.MultipartForm.RemoveAll()`.
- **Zero-Allocation отправка WebSocket-кадров**:
  - Сборка заголовков кадра на стеке с прямой записью в буфер сокета без создания временных срезов в куче.
- **Сбор системных метрик без аллокаций**:
  - Потоковый разбор `/proc/meminfo` без создания `map[string]int64`, срезов и отражения каждые 2 секунды.
  - Кэширование физического объема памяти роутера и устранение дублирующего обхода `/proc` в диспетчере служб.
- **Файловый менеджер и архивация**:
  - Внедрен пул буферов `copyBufPool` (`sync.Pool` по 32 КБ) для файловых операций.
  - Автоматический возврат памяти ядру ОС после создания и распаковки архивов и бэкапов.
- **Логирование**:
  - Кольцевой буфер истории в памяти сокращен до 250 записей с автоматической ротацией лог-файла в `/tmp` при достижении 256 КБ для защиты `tmpfs`.

## [1.0.30] - 2026-09-11

### Отказоустойчивость установщика (install.sh) и поддержка HTTPS
- **Автоматическая установка `wget-ssl` и сертификатов**:
  - Установщик определяет наличие поддержки HTTPS в Entware. При использовании стандартного BusyBox `wget` автоматически устанавливает `wget-ssl`, `ca-certificates` и `ca-bundle` из базового HTTP-репозитория до подключения кастомного фида.
  - Настройка `option wget_cmd /opt/bin/wget` и приоритет `/opt/bin` в `$PATH` исключают ошибку `wget: not an http or ftp url`.
- **Безопасная проверка API и устранение `Segmentation fault`**:
  - Предварительная валидация запуска `curl` и безопасный опрос `/api/status` с защитой от аварийных завершений на ядрах aarch64 3.10.
  - Мгновенное прерывание скрипта с информативной ошибкой при сбое команды установки `opkg install`.

### Интеллектуальное автоопределение SSH-порта и диагностика
- **Расширенное детектирование SSH-порта роутера**:
  - Сканирование командной строки запущенных процессов Dropbear / SSHD в `/proc`.
  - Анализ слушающих сокетов в `/proc/net/tcp` и `/proc/net/tcp6`, разбор скриптов автозапуска и конфигураций OpenSSH.
- **Централизованный модуль отправки отчетов об ошибках (Bug Report)**:
  - Автоматический сбор самодиагностики роутера с маскированием конфиденциальных данных (пароли, токены).
  - Удобная отправка баг-репорта в репозиторий GitHub в один клик.

### Интерфейс и интеграция с сообществом
- Добавлены ссылки на официальный Telegram-канал (@SmartKeenetic) и тему на форуме Keenetic.
- Улучшения двухпанельного файлового менеджера и системного анализатора.
