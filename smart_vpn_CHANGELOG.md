...
## [1.0.119] - 2026-09-15
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Обновленный UX, переименование серверов и раздельная маршрутизация**:
  - **3 режима маршрутизации VPN**: «Все в VPN», «По приложениям», «По спискам».
  - **2 режима маршрутизации SOCKS5**: «Все в VPN», «По спискам».
  - **Детект ТСПУ / DPI с авто-Fallback**: Автоматический перехват заблокированных ТСПУ прямых TCP-соединений с мгновенным перенаправлением в VPN-туннель.
  - **Переименование серверов**: Возможность задавать пользовательские названия серверов и отображение их в карточках.
  - **Индикация в режиме Прокси**: Отображение внешнего IP и задержки Ping в режиме только Прокси.
  - **Улучшенный визуальный стиль**: Лаконичные статусы без лишних префиксов и надписей.

## [1.0.116] - 2026-09-15
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Устранение сброса сессий и зависания SOCKS5 на IPv6/недоступных адресах**:
  - **Мгновенный отказ Host Unreachable для неработающих адресов**: При невозможности подключения к конкретному IP датацентра или IPv6-адресу через VPN туннель прокси теперь возвращает SOCKS5 ошибку `Host Unreachable` (0x04) вместо ошибочной отправки на прямой сокет оператора РФ (`DIRECT`), на котором трафик блокируется ТСПУ. Клиент Telegram мгновенно переключается на рабочие IPv4 датацентры без зависаний.
  - **Устранение ложного сброса VPN-сессии при таймауте виртуальных потоков**: Удален ошибочный вызов `stop()` в `openStream` при таймауте единичного потока к удаленному хосту.
  - **Стабилизация проверки активности туннеля (Keepalive)**: Увеличен порог проверки тишины сервера до 30 секунд с защитой от ложных срабатываний при длительном ожидании сообщений (idle-соединениях).

## [1.0.115] - 2026-09-15
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Исправление подключения Telegram и приложений через SOCKS5/HTTP прокси (:10808)**:
  - **Маршрутизация SOCKS5 по умолчанию через VPN**: Поскольку SOCKS5 настраивается пользователем точечно в конкретных приложениях (Telegram, Nekogram и др.), трафик SOCKS5 теперь гарантированно направляется в VPN туннель (`RouteDecision.VPN`), исключая ошибочную отправку на заблокированные в РФ IP датацентров Telegram напрямую (`DIRECT`).
  - **Прямой виртуальный поток OpcodeStream через VPS**: Подключение через SOCKS5/HTTP прокси принудительно использует прямой легковесный виртуальный поток (`session.openStream`), который пробивается напрямую через зашифрованный туннель VPS в обход ограничений системного TUN и политик раздельного туннелирования.
  - **Полный список подсетей Telegram в пресетах SmartRoute**: В пресет `telegram_full` добавлены недостающие диапазоны датацентров Telegram (`194.221.250.0/24`, `149.154.160.0/20`, `91.108.0.0/16`, `185.76.151.0/24`) и пресет включен по умолчанию.
  - **Улучшенный SOCKS5 рукопожатие и чтение пакетов**: Внедрена функция `readFully` для гарантированного считывания байтов рукопожатия аутентификации, длин доменных имен и адресов IPv4/IPv6 без фрагментации.
  - **Сквозное двустороннее закрытие сокетов**: Исправлена синхронизация потоков ретрансляции `relayStream` и `relaySockets`, предотвращающая зависание сессий и утечки активных соединений при разрыве связи с удаленным сервером.

## [1.0.114] - 2026-09-15
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Улучшения раздельного туннелирования, фоновой работы и UX**:
  - **Исправление диалога оптимизации батареи**: Системный запрос исключения из режима энергосбережения теперь запрашивается строго однократно при первом запуске, не открываясь назойливо при каждом входе в приложение. В «Настройки» добавлен прямой пункт «Работа в фоне и батарея».
  - **Автоматический выбор заблокированных в РФ приложений**: В режиме «Только выбранные» по умолчанию автоматически отмечаются обнаруженные на устройстве сервисы из черного списка РКН (YouTube, YouTube Music, Instagram, X/Twitter, Discord, Telegram, ChatGPT, Claude, Copilot, Spotify, Canva, Notion, LinkedIn, Meduza, Proton). Добавлена кнопка быстрого применения пресета «⚡ Заблокированные в РФ».
  - **Главный переключатель раздельного туннелирования (Вкл/Выкл)**: Реализован явный Master Switch на экране раздельного туннелирования. При выключенном туннелировании 100% интернет-трафика всех приложений направляется через VPN.
  - **Исправление геометрии вкладки «По спискам»**: Устранен визуальный баг растягивания переключателя вкладок `RoutingSubTabRow` на всю ширину экрана; обеспечены согласованные отступы `16dp` на обеих субвкладках.
  - **Удалена служебная плашка «Совмещенный режим»**: Экран раздельного туннелирования очищен от избыточных информационных баннеров.
  - **Копирование прокси для APN**: В диалоге конфигурации добавлена кнопка «Скопировать прокси и открыть APN», мгновенно помещающая `127.0.0.1:10808` в буфер обмена.

## [1.0.113] - 2026-09-15
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Единый Mixed-прокси (HTTP + SOCKS5 на одном порту :10808)**:
  - **Объединение портов в Mixed-режим**: Реализован встроенный смешанный прокси-сервер (по аналогии с Sing-Box и Clash), слушающий один TCP-порт (`127.0.0.1:10808` по умолчанию).
  - **Автоматическое распознавание протокола по первому байту**:
    - При первом байте `0x05` соединение мгновенно направляется в обработчик **SOCKS5** (Telegram, SOCKS-клиенты).
    - При текстовых ASCII-байтах (`CONNECT`, `GET`, `POST` и др.) поток обрабатывается как **HTTP / HTTPS CONNECT** прокси (браузеры, curl, Wi-Fi настройки Android).
    - Использован `PushbackInputStream` со сквозной трансляцией потоков без задержек и накладных расходов.
  - **Упрощение интерфейса и настроек**: В настройках приложения и диалоге конфигурации вместо двух разрозненных полей (HTTP и SOCKS5) теперь отображается единый порт с подсказкой о поддержке обоих протоколов.
  - **Полная обратная совместимость**: Сохранены свойства `proxyHttpPort` и `proxySocksPort`, маппящиеся на единый порт.

## [1.0.112] - 2026-09-14
### Добавлено и Улучшено
- **QuakeLive-VPN и Smart-Zapret — Выбор номера OpkgTun / OpkgTap слотов и автоподбор свободного**:
  - **Интерактивный выбор слотов (0..9 и Авто)**: В веб-интерфейс QuakeLive-VPN и Smart-Zapret добавлен выпадающий селектор слотов NDM (`OpkgTun` и `OpkgTap`) со статусом каждого слота («Свободен» / «Занят: Описание»).
  - **Режим «Авто (первый свободный)» по умолчанию**: При запуске туннелей обе подсистемы автоматически анализируют занятые интерфейсы KeeneticOS NDM (`show interface`) и активные Linux-адаптеры ядра, выбирая первый гарантированно свободный слот.
  - **Изоляция и защита от конфликтов с системными сервисами**: Исключены конфликты с занятыми слотами `OpkgTun0` (QuakeLive-VPN), `OpkgTun1` (Sing-Box / Policy-TUN) и `OpkgTun2` (SmartRoute BlackHole).
  - **API управления слотами**: Реализованы эндпоинты `GET /api/ndm/opkg-slots`, `POST /api/qlvpn/client/iface-mode` (с поддержкой `interface_slot`), `POST /api/smartzapret/iface-slot`.

## [1.0.111] - 2026-09-14
### Добавлено и Улучшено
- **Smart-Zapret — Поддержка системных интерфейсов KeeneticOS NDM и режимов OpkgTun / OpkgTap**:
  - **Интеграция с KeeneticOS NDM (`ndmc`)**: Smart-Zapret теперь полноценно регистрируется как системный интерфейс роутера Keenetic (`OpkgTun3` / `OpkgTap3`), отображаясь в веб-интерфейсе KeeneticOS, графиках мониторинга трафика и политиках маршрутизации («Приоритеты подключений»).
  - **Выбор режима интерфейса на лету**:
    - **OpkgTun (L3, по умолчанию)**: системный TUN-интерфейс `opkgtun3` со сквозной регистрацией в NDM для L3 маршрутизации.
    - **OpkgTap (L2)**: виртуальный TAP-интерфейс `opkgtap3` со встроенным ARP-ответчиком (Zero External Dependencies) для прозрачной интеграции в Ethernet-мосты и L2 политики KeeneticOS.
    - **Standard (Linux raw TUN)**: изолированный виртуальный адаптер `smartzapret-tun` без регистрации в KeeneticOS NDM.
  - **Панель управления интерфейсом в веб-интерфейсе**:
    - Интерактивный селектор режимов (`OpkgTun` / `OpkgTap` / `Standard`) с мгновенным переключением через API `POST /api/smartzapret/iface-mode`.
    - Отображение статуса регистрации в NDM и активного системного имени адаптера.
  - **Безопасное управление жизненным циклом**: автоматическая дерегистрация и корректное опускание (`down`) интерфейса в NDM при остановке службы Smart-Zapret.

## [1.0.110] - 2026-09-14
### Добавлено и Улучшено
- **Реорганизация навигации веб-интерфейса**:
  - Каталог клиентов и протоколов (Roadmap) перенесен в качестве третьей субвкладки **«📦 Каталог клиентов»** внутрь раздела **«Другие VPN»** (`Sing-Box • AmneziaWG • Каталог`).
  - Очищена главная панель навигации роутера, обеспечив более компактное и логичное отображение управляющих вкладок.
  - Сохранена полная совместимость с глубокими ссылками по хешу `#catalog`.

## [1.0.109] - 2026-09-14
### Добавлено и Улучшено
- **Smart-Zapret — Встроенный автономный Anti-DPI движок на чистом Go (Zero CGO, Zero External Dependencies)**:
  - **Архитектура обхода без VPS**: реализован полноценный локальный Anti-DPI движок прямо на роутере Keenetic без необходимости аренды удаленного VPS-сервера.
  - **Входной виртуальный адаптер L3 TUN**: создание устройства `smartzapret-tun` с назначением локальной IPv4-подсети (`10.254.1.1/24`) и перехватом входящего интернет-трафика.
  - **Многостратегический анализатор и модификатор пакетов (Mangler)**:
    - **Split SNI**: расщепление TCP payload на уровне TLS ClientHello (SNI) с сохранением непрерывности порядковых номеров Seq/Ack.
    - **Fake SNI + Low TTL**: инъекция предварительного поддельного TLS ClientHello (например, `gosuslugi.ru`) с уменьшенным временем жизни (TTL 3–6), сбивающая алгоритмы сигнатурного анализа ТСПУ.
    - **TCP Disorder**: отправка второго TCP-сегмента раньше первого с микрозадержкой для обхода DPI-буферов.
    - **QUIC / HTTP/3 Drop**: выборочный сброс UDP 443 пакетов для форсирования перехода браузеров и клиентов на устойчивый модифицированный TCP TLS.
    - **Расчет контрольных сумм RFC 1071**: потоковый расчет IPv4 и TCP pseudo-header checksum в чистом Go без CGO.
  - **Прямая отправка в WAN через RAW Socket**: передача модифицированных кадров напрямую в шлюз роутера через сырой сокет `syscall.SOCK_RAW` (`IP_HDRINCL`, `SO_BINDTODEVICE`).
  - **1-Click Пресеты обхода**:
    - *YouTube 4K Fix* (Split SNI + Drop QUIC)
    - *Discord Voice & Chat Fix* (Fake SNI + TTL 4)
    - *Universal Anti-TSPU* (Fake SNI + Split SNI + Disorder)
    - *Пользовательский режим (Custom)* с ручной регулировкой TTL, смещения и задержки.
  - **Веб-интерфейс и телеметрия в реальном времени**:
    - Новая вкладка `⚡ Smart-Zapret` с метриками обработанных пакетов, инъекций Fake SNI, модифицированных рукопожатий и сброшенных QUIC.
    - Интеграция с Каталогом клиентов и сквозным роутингом хешей URL.

### Добавлено и Улучшено
- **QuakeLive-VPN Android — Упрощение интерфейса и удаление неиспользуемого режима анти-детекта**:
  - Из экрана настроек и карточки локального прокси полностью удален переключатель «Анти-детект (Без системного TUN)», исключив путаницу в интерфейсе.
  - Очищены параметры сессии, состояние приложения и фоновые слушатели настроек от рудиментов режима без TUN.

## [1.0.107] - 2026-09-14
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Защита от потери связи в прокси, прямой переход в APN и верстка полей поиска**:
  - **Прямой переход в настройки APN**: кнопка в `ProxyConfigDialog` переведена на вызов нативного намерения `Settings.ACTION_APN_SETTINGS` с цепочкой запасных системных переходов, открывая параметры точек доступа оператора напрямую без перехода в промежуточное меню «Другие способы подключений».
  - **Отказоустойчивость локального прокси (DIRECT Fallback)**: при недоступности VPN-канала или задержке ответа туннеля локальный прокси-сервер автоматически перенаправляет запросы напрямую (`DIRECT` через защищенный сокет) вместо возврата ошибки `502 Bad Gateway` / `Connection refused`. Это предотвращает блокировку интернета на телефоне при временных сбоях туннеля у пользователей со сконфигурированным системным прокси.
  - **Устранение обрезки полей поиска**: поля ввода на вкладках «По приложениям» и «По спискам» переведены на `BasicTextField` с вертикальным центрированием, полностью исключив обрезку нижних выносных элементов букв в placeholder (`Поиск приложений...`, `Поиск списков...`).
  - **Очистка настроек**: из карточки параметров локального прокси удалена избыточная плашка «Гибридный режим».

## [1.0.106] - 2026-09-14
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Оптимизация главного экрана и полей поиска**:
  - **Упрощение главного экрана**: с центральной зоны под эмблемой удалена избыточная кнопка «Параметры и настройка в Android».
  - **Компактный виджет прокси**: карточка «VPN + ЛОКАЛЬНЫЙ ПРОКСИ» переименована в лаконичное «ПРОКСИ» и сжата в аккуратную двухстрочную панель (адрес прокси-портов + бейдж активных соединений с настройкой `⚙️` в одну строку), исключив дублирование управляющих кнопок.
  - **Оптимизация поисковой строки в «По приложениям»**: убран длинный список примеров (`ю, youtube, сбер...`), вызывавший обрезку текста на узких экранах; заменен на чистый и компактный placeholder `Поиск приложений...`.
  - **Оптимизация поисковой строки в «По спискам»**: placeholder сокращен до `Поиск списков...` с защитой от переноса и обрезки; из подзаголовка полностью удален устаревший префикс `SmartRoute:`.

## [1.0.105] - 2026-09-14
### Добавлено и Улучшено
- **QuakeLive-VPN — Самовосстановление сессий (Self-Healing) и двусторонний Keepalive**:
  - **Двусторонний Keepalive (Server & Client)**: сервер `ql-vpn` теперь эхом отвечает на кадры `OpcodeKeepalive` (0x01) зашифрованными пакетами, поддерживая активность NAT-трансляций и актуальность сессионных ключей в обоих направлениях.
  - **Автоматическое обнаружение сброса/перезапуска сервера**: клиент Android непрерывно отслеживает таймер входящих пакетов от сервера (`lastServerRxTime`). При обнаружении потери связи (нет ответов более 12 секунд при активном трафике/keepalive или таймаут открытия потока `openStream` более 5 секунд) клиент автоматически сбрасывает устаревшую сессию и выполняет мгновенный 1-RTT handshake (300 мс) без участия пользователя.
  - **Защита батареи устройства в режиме ожидания**: таймер таймаута привязан к флагу подтверждения двустороннего Keepalive (`hasServerKeepaliveEcho`) и наличию активного трафика, исключая ложные переподключения и пробуждения процессора телефона в состоянии покоя.
  - **Динамический замер сетевой задержки (Live RTT Ping)**: клиент в реальном времени рассчитывает точный RTT пинг по времени прохождения подтвержденных сервером исходящих номеров последовательности (`sentSeqTimes`).

## [1.0.104] - 2026-09-14
### Добавлено и Улучшено
- **QuakeLive-VPN — Гибридная маршрутизация локального прокси и поддержка потокового туннелирования (Anti-Detect Stream Proxy)**:
  - **Гибридный роутинг сокетов в Android**: если системный VPN активен (`isTunActive`), прокси направляет трафик напрямую через TUN интерфейс туннеля в ядро системы с нулевым оверхедом; в чистом прокси-режиме (без TUN) используется виртуальный поток `OpcodeStreamOpen`.
  - **Безопасное дробление MTU (1200 байт)**: в `sendStreamData` реализовано автоматическое разбиение полезной нагрузки на порции по 1200 байт для предотвращения фрагментации и сброса UDP пакетов сотовыми операторами.
  - **Серверная подсистема виртуальных TCP потоков (Linux amd64/arm64)**: обновлен и скомпилирован сервер `ql-vpn` с полноценной обработкой потоков `OpcodeStreamOpen`, `OpcodeStreamData`, `OpcodeStreamClose`.
  - **Скрипты развертывания**: в `install_vps.sh` обновлена ссылка загрузки бинарных файлов на репозиторий `Keenetic/main/bin/`.

## [1.0.103] - 2026-09-14
### Добавлено и Улучшено
- **QuakeLive-VPN Android — Постоянный локальный прокси и синхронизация списков**:
  - Исправлена синхронизация списков правил SmartRoute: `RoutingListRepository` преобразован в потокобезопасный Singleton с непрерывным отслеживанием Flow изменений списков. Теперь добавленные домены (например, `2ip.io`) моментально применяются к фильтрации трафика локального прокси.
  - Улучшен сопоставитель доменов `RuleMatcher`: автоматическая нормализация записей доменов (удаление протоколов `https://`, портов, путей, масок `*.` и `||`) и точное сопоставление поддоменов.
  - Добавлено диалоговое окно быстрой настройки Android (`ProxyConfigDialog`) с кнопками быстрого перехода в настройки Wi-Fi и мобильных сетей (APN), а также копированием параметров в один клик.
  - Добавлены кнопки быстрого перехода к настройке прокси на главном экране (под эмблемой и в карточке активного прокси) и в настройках клиента.
  - В карточку скорости и телеметрии добавлен счетчик суммарного объема переданных и принятых данных за сессию (`↓ RX • ↑ TX`).

## [1.0.99] - 2026-09-11
### Добавлено и Улучшено
- **Генерация QR-кодов для клиентов сервера QuakeLive-VPN**:
  - В таблицу выданных токенов сервера (`Список выданных токенов клиентов`) добавлена кнопка **«📱 QR»** для каждого клиента.
  - В список сохраненных серверов клиента добавлена кнопка **«📱 QR»** для быстрого экспорта подключения на телефон.
  - В форму 1-Click развертывания на удаленный VPS по SSH (`deploy`) добавлена кнопка **«📱 QR»** для сканирования полученного root-токена.
  - При создании нового токена клиента в веб-панели теперь автоматически открывается модальное окно с готовым QR-кодом для быстрого подключения.
  - Реализовано стилизованное модальное окно QR-кода с отображением имени клиента, адреса сервера, выделенного IP, полным текстом токена, кнопкой копирования и подсказкой для мобильного сканера.
  - **100% автономность (Zero CDN)**: встроен легковесный автономный генератор QR-кодов в векторном формате SVG (`qrcode.min.js`), работающий полностью оффлайн без внешних запросов к сети.
  - Поддержка отображения QR-кодов внедрена как в основной веб-интерфейс Smart-VPN (роутеры Keenetic), так и в автономную веб-панель QuakeLive-VPN (Linux VPS и Windows).
- **Синхронизация версий**:
  - Обновлены версии всех компонентов до единой сквозной версии v1.0.99 (Android `versionCode = 99`).

## [1.0.98] - 2026-09-11
### Добавлено и Улучшено
- **Стандартизация портов по умолчанию**:
  - Порт веб-интерфейса `Smart-VPN` по умолчанию во всех операционных системах зафиксирован как **8091**.
  - Порт веб-админки `QuakeLive-VPN` (сервер, VPS, клиент) по умолчанию во всех системах зафиксирован как **8092**.
- **Настройка Bind-Address (сетевого адреса привязки веб-интерфейса)**:
  - В `Smart-VPN` и `QuakeLive-VPN` добавлен параметр `bind-address` (`web_bind_addr`).
  - По умолчанию на **Windows** используется безопасная локальная привязка **`127.0.0.1`**.
  - На **Linux / KeeneticOS / VPS** по умолчанию используется **`0.0.0.0`** (доступ из локальной сети или извне).
  - Параметр доступен для настройки в веб-интерфейсе обоих сервисов, а также через CLI-флаг `-bind` в `ql-vpn`.
- **Контроль доступности порта при запуске на Windows**:
  - Реализован предварительный тест связывания порта админки (`CheckAdminPortAvailable`).
  - В случае конфликта портов (порт уже занят другим процессом) на Windows выводится нативное диалоговое окно `MessageBoxW` с подробными инструкциями и командами для диагностики и освобождения порта (`netstat -ano | findstr :<port>`, `taskkill /F /PID <PID>`), а также указанием CLI-флага для запуска на другом порту (`-web-port <порт>`).
- **Синхронизация версий**:
  - Обновлены версии клиента и сервера QuakeLive-VPN и Smart-VPN до v1.0.98, versionCode Android-клиента поднят до 98.

## [1.0.97] - 2026-09-11
### Добавлено и Улучшено
- **Релизный Windows GUI клиент QuakeLive-VPN (`ql-vpn.exe`) в общем репозитории**:
  - Выполнена чистая релизная сборка клиента для Windows x64 (`-trimpath -ldflags "-s -w -H=windowsgui"`) без всплывающих консольных окон при запуске.
  - Windows-клиент (`ql-vpn.exe` и `ql-vpn-windows-amd64.exe`) опубликован в общий репозиторий `snakelair/Keenetic` (`bin/`).
  - В скрипт сборки пакетов `scripts/build_packages.go` добавлена автоматическая синхронизация релизных сборок под Windows в каталог общего репозитория.
  - На главной витрине репозитория `Keenetic/index.html` добавлены прямые кнопки скачивания релизных клиентов под Windows (`.exe`) и Android (`.apk`).
- **Синхронизация версий**:
  - Обновлены версии клиента и сервера QuakeLive-VPN и Smart-VPN до v1.0.97, а также versionCode Android-клиента (97).

## [1.0.96] - 2026-09-11
### Добавлено и Улучшено
- **Android-клиент QuakeLive-VPN — Система проверки обновлений и самообновления из общего репозитория**:
  - Внедрен компонент `AppUpdateManager` для проверки обновлений через общий репозиторий `snakelair/Keenetic` (`apk/version.json`).
  - Фоновая ненавязчивая проверка при запуске приложения и ручная проверка в разделе «Настройки» -> «О приложении».
  - Потоковое скачивание APK с индикацией прогресса (проценты, МБ) и автоматический запуск установки через системный `FileProvider` (`REQUEST_INSTALL_PACKAGES`).
  - Полнофункциональное модальное окно `AppUpdateDialog` со списком изменений (Changelog) и возможностью перехода по прямой ссылке в браузере.
  - В общий репозиторий `Keenetic/apk/` настроена автоматическая выгрузка исключительно подписанных релизных APK (`QuakeLiveVPN.apk`), защищенных приватным производственным ключом.
- **VPS / QuakeLive-VPN — Исправление автоматической перезагрузки после обновления**:
  - Устранена необходимость ручного нажатия клавиши F5 в веб-интерфейсе после выполнения самообновления на VPS.
  - Алгоритм опроса статуса демона теперь надежно распознает факт временного падения службы при перезапуске и ее последующее успешное поднятие.
  - Внедрена автоматическая принудительная перезагрузка страницы со сбросом браузерного кэша (`window.location.replace` с меткой времени `?_v=timestamp`).
  - В скриптах VPS добавлена поддержка алиасов systemd-юнитов `qlvpn` и `ql-vpn`.

## [1.0.95] - 2026-09-11
### Добавлено и Улучшено
- **Android-клиент QuakeLive-VPN — Приватный производственный Keystore и релизная подпись**:
  - Создан официальный закрытый ключ подписи релизных APK (`android/release.keystore`, PKCS12 / RSA 4096 бит, срок действия 10 000 дней до 2054 года, `CN=QuakeLive VPN, OU=SnakeLair, O=SnakeLair, C=RU`).
  - В `build.gradle.kts` добавлена автоматическая загрузка параметров подписи из `keystore.properties` с поддержкой схем APK Signature Scheme v1 и v2.
  - Релизный вариант сборки переведен с отладочного тестового сертификата `Android Debug` на полноценную production-подпись с автоматическим fallback при отсутствии ключа.
  - Пересобран и проверен официальный релизный дистрибутив `QuakeLiveVPN.apk`.

## [1.0.94] - 2026-09-10
### Изменено
- **Обновление ссылок на Telegram-сообщество**:
  - Все ссылки на Telegram-канал и чат в веб-интерфейсе роутера, VPS-установщике, приложении Android (Kotlin), QuakeLive-VPN и генераторе баг-репортов переведены на новый официальный адрес: [`t.me/SmartKeenetic`](https://t.me/SmartKeenetic).

## [1.0.93] - 2026-09-10
### Исправлено
- **QuakeLive-VPN / VPS — Синхронизация версий и исправление отображения v1.0.88**:
  - Устранено расхождение между переменными версии пакета `qlvpn` (`internal/qlvpn/version.go`) и точкой входа `main` (`cmd/ql-vpn/main.go`), из-за которого после обновления на VPS в веб-интерфейсе ошибочно отображалась устаревшая версия `v1.0.88`.
  - Добавлена автоматическая синхронизация `qlvpn.Version = Version` в `init()` при запуске процесса QuakeLive-VPN.
  - В скриптах компиляции (`build_packages.go`, `build.bat`, `build.ps1`) флаги линкера (`-ldflags`) теперь явно прописывают версию как в `main.Version`, так и в `github.com/snakelair/SmartVpn/internal/qlvpn.Version`.
  - Реализована динамическая подстановка актуальной версии в HTML-шаблон веб-интерфейса VPS при отдаче корневой страницы.
  - Синхронизированы номера версий клиента Android (Kotlin / Gradle).

## [1.0.92] - 2026-09-10
### Исправлено
- **Веб-интерфейс / QuakeLive-VPN — Исправление переключения субвкладок**:
  - Устранена синтаксическая ошибка в `qlvpn.js` (пропущенная закрывающая фигурная скобка метода `createToken`), приводившая к сбою компиляции скрипта в браузере и невозможности инициализации глобального объекта `window.QLVpn`.
  - Восстановлена полноценная работоспособность кнопок переключения субвкладок «Клиент QL-VPN», «Сервер QL-VPN» и «Развернуть на VPS по SSH».

## [1.0.91] - 2026-09-10
### Добавлено и Улучшено
- **QuakeLive-VPN — Управление автозапуском, мульти-клиент и отображаемые имена**:
  - **Отключение автозапуска сервера по умолчанию**: Сервер QuakeLive-VPN теперь отключен по умолчанию на всех платформах (включая роутер). Текущее состояние (включен/выключен) надежно запоминается в `qlvpn-server.json` при каждом ручном запуске или остановке.
  - **Отображаемое имя сервера (Server Display Name)**:
    - Серверу можно задать понятное отображаемое имя (по умолчанию имя роутера или компьютера).
    - Заданное имя автоматически включается в генерируемый токен доступа `qlvpn://` (поле `server_name`).
    - Имя отображается в карточке клиента при импорте токена и в настройках сервера.
  - **Пользовательское переименование подключений**:
    - Имя сервера из токена по умолчанию используется как название клиентского подключения.
    - В карточки серверов добавлена кнопка переименования подключения ✏️ с возможностью задать произвольное имя (`display_name`).
  - **Одновременное подключение к нескольким серверам (Multi-Client)**:
    - Реализована возможность одновременного подключения к нескольким серверам на всех платформах (KeeneticOS, Linux, Windows, macOS; кроме Android из-за системного ограничения `VpnService`).
    - Независимое управление и отключение для каждого активного туннеля.
    - Динамическое выделение свободных сетевых интерфейсов (`opkgtun0`, `opkgtun2`, `opkgtun3`... на роутере с резервированием `opkgtun1` под Sing-Box; множественные адаптеры Wintun `QL-VPN`, `QL-VPN-2`... на Windows).
  - **Приоритеты маршрутов (Route Metric Priority)**:
    - При наличии 2 и более активных подключений в веб-интерфейсе отображается панель управления приоритетами маршрутов.
    - Пользователь может кнопками ▲ / ▼ перераспределять приоритеты активных туннелей.
    - Метрики сетевых маршрутов (`metric 100`, `metric 200`... на Linux; `metric 10`, `metric 20`... на Windows) автоматически обновляются на сетевых интерфейсах ОС в реальном времени.

## [1.0.90] - 2026-09-10
### Исправлено и Улучшено
- **Веб-интерфейс / Вкладка «Другие VPN» — Исправление отображения AmneziaWG (Userspace)**:
  - **Устранение ошибочной вложенности DOM**: Исправлен пропущенный закрывающий тег `</div>` субпанели Sing-Box (`#other-subpane-singbox`), из-за которого блок AmneziaWG (`#other-subpane-amneziawg`) оказался вложен внутрь Sing-Box и скрывался при переключении субвкладок, вызывая пустой экран.
  - **Корректная инициализация формы при отсутствии конфигов**: Устранена попытка загрузки несуществующего `amneziawg.conf` с ошибкой 404. Добавлена функция безопасной генерации дефолтной конфигурации (`getDefaultConfig`) с автоматическим заполнением формы и селектора.

## [1.0.89] - 2026-09-10
### Добавлено и Улучшено
- **Полноценная поддержка Userspace AmneziaWG (amneziawg-go + awg-tools + opkgtun)**:
  - **Интеграция amneziawg-go**: Добавлена встроенная поддержка userspace-клиента `amneziawg-go` и утилиты `awg-tools` (`awg`, `awg-quick`) для работы протокола AmneziaWG на любых ядрах Linux KeeneticOS без требования наличия встроенного модуля ядра `amneziawg.ko`.
  - **Поддержка протоколов AWG 1.0, 2.0 и AWG 3.1**: Реализована полная поддержка всех параметров обфускации:
    - AWG 1.0 / 2.0: `Jc`, `Jmin`, `Jmax`, `S1`, `S2`, `S3`, `S4`, `H1`, `H2`, `H3`, `H4`, `I1`.
    - AWG 3.1: `HeaderProtectionKey`, `ContentPaddingAddition`, `RandomTrailers`, `DisableCookies`.
  - **Управление конфигурациями**: Поддержка добавления, редактирования, валидации и удаления конфигураций `.conf`, автоматический парсинг и генерация конфигов для `amneziawg-go` / `awg-quick`.
  - **Автоматическая установка и детектор архитектуры**: Определение наличия бинарников на роутере и автоматическая установка пакетов `amneziawg-go` и `awg-tools` через `opkg` под архитектуру роутера (`mipsle-softfloat`, `armv7`, `arm64`, `x86_64`).
  - **Живая телеметрия и логи**: Мониторинг передачи байт (Rx/Tx), последнего хэндшейка пиров и вывод системных журналов работы демона в реальном времени.
- **Универсальная поддержка OpkgTun / OpkgTap и интеграция с KeeneticOS NDM для всех сторонних клиентов**:
  - **Единый модуль `internal/ndm`**: Создан централизованный пакет управления сетевыми интерфейсами KeeneticOS (`RegisterNDMInterface`, `SetNDMInterfaceDown`, `DeleteNDMInterface`, `RunNDMC`).
  - **Sing-Box на OpkgTun / OpkgTap**: В конфигурацию Inbound TUN Sing-Box добавлена возможность выбора интерфейса (`opkgtun1`, `opkgtap1` или стандартный `singbox-tun`). При запуске интерфейс регистрируется в NDM (`interface OpkgTun1`), становится доступным в политиках маршрутизации KeeneticOS (Multi-WAN), а при остановке переводится в `down` без сброса назначенных правил и политик.
  - **AmneziaWG на OpkgTun**: Интерфейс `opkgtun0` (или `opkgtun2`) автоматически регистрируется в NDM при запуске туннеля и плавно деактивируется при остановке.
  - **Рефакторинг QuakeLive-VPN**: Взаимодействие с NDM переведено на единую реализацию пакета `internal/ndm`.
- **Реорганизация веб-интерфейса — Единая вкладка «Другие VPN» (`#other-vpn`)**:
  - Все сторонние VPN-клиенты и ядра (Sing-Box, AmneziaWG Userspace) сгруппированы в общую навигационную вкладку **«Другие VPN»** с быстрыми субвкладками переключения.
  - **QuakeLive-VPN** сохранен в виде выделенной самостоятельной вкладки на панели навигации.
  - Добавлены селекторы и индикаторы привязки интерфейсов NDM (OpkgTun / OpkgTap / Standard) с подсказками по приоритетам подключений KeeneticOS.

## [1.0.88] - 2026-09-07
### Добавлено и Улучшено
- **Раздел «💖 Поблагодарить автора & Поддержка проекта» во всех приложениях**:
  - **Роутер (Keenetic Web UI)**: В главное меню добавлена навигационная вкладка «Поблагодарить автора» (`#donate`) со стилизованными карточками ЮMoney (с копированием кошелька и быстрыми суммами), Boosty.to, Т-Банк (СБП) и ссылками на сообщество. В настройках добавлена кнопка быстрого перехода.
  - **Windows-клиент & VPS-панель (`embeddedHTML`)**: В общую панель управления встроена субвкладка «💖 Поблагодарить автора», а также пункт меню в системном трее Windows.
  - **Android APK**: В экран «Настройки» добавлен блок «ПОДДЕРЖКА ПРОЕКТА», а в шапку экрана «Дашборд» — кнопка с сердечком для быстрого открытия окна `DonateModal` со всеми платежными реквизитами и СБП.
  - **VPS скрипт установки**: В консольный отчет после установки `install_vps.sh` добавлен блок поддержки автора и ссылок на проект.

## [1.0.87] - 2026-09-07
### Исправлено и Улучшено
- **QuakeLive-VPN / KeeneticOS NDM — Устранение дублирования интерфейсов OpkgTun**:
  - **Устранение гонки интерфейсов**: Из функции `OpenTun` удален ошибочный алгоритм перебора свободных интерфейсов (`opkgtun0` -> `opkgtun1` -> `opkgtun2`...), который при временной занятости `opkgtun0` в момент реконнекта создавал новые интерфейсы и перезаписывал сторонние туннели NDM (например, `OpkgTun1` / `awgm policy-tun`).
  - **Повторные попытки при EBUSY**: Добавлен цикл ожидания с экспоненциальной паузой (до 5 попыток по 100 мс) на строго заданном имени интерфейса, дающий ядру Linux время корректно освободить файловый дескриптор.
  - **Безопасная переинициализация**: В `ConnectClient` внедрена задержка 50 мс после закрытия существующего туннеля для полного освобождения ресурсов netdev перед открытием нового сеанса.
  - **Очистка NDM**: На роутере восстановлено исходное описание `OpkgTun1` (`awgm policy-tun`), исключив дублирующиеся подключения `QuakeLive-VPN (SnakeLair)` в родном веб-интерфейсе KeeneticOS.

## [1.0.86] - 2026-09-07
### Изменено и Унифицировано
- **Ребрендинг названия приложения: Smart-VPN**:
  - **Унификация через дефис**: Название сервиса стандартизировано как **Smart-VPN** во всем веб-интерфейсе роутера, фоновой службе, init-скриптах Entware (`S99smart-vpn`), API заголовках, логах и NetChan баннерах QuakeLive-VPN.
  - **Веб-интерфейс**: Обновлен заголовок страниц (`<title>`), главный логотип в шапке, модальные окна, вкладки «Журнал событий» и «Настройки приложения».
  - **Системная служба**: Описание службы в NDM/Entware и вывод при старте/остановке обновлены на `Smart-VPN Keenetic Control Center`.

## [1.0.85] - 2026-09-07
### Исправлено и Улучшено
- **QuakeLive-VPN / KeeneticOS NDM — Сохранение интерфейса и политик маршрутизации при остановке**:
  - **Перевод в DOWN вместо удаления**: При остановке или отключении клиента интерфейс `OpkgTun0` / `OpkgTap0` больше не удаляется из NDM (`no interface ...`), а просто переводится в состояние `down` (`interface OpkgTun0 down`).
  - **Сохранение пользовательских настроек**: Назначенные в веб-интерфейсе KeeneticOS политики маршрутизации (Приоритеты подключений / Policy-based routing), привязки домашних устройств и правила файрвола полностью сохраняются и не сбрасываются при перезапуске туннеля.

## [1.0.84] - 2026-09-07
### Добавлено и Улучшено
- **QuakeLive-VPN — Нативная интеграция с KeeneticOS NDM (OpkgTun / OpkgTap)**:
  - **Автоматическая регистрация в NDM**: Клиент QuakeLive-VPN при запуске на роутерах Keenetic теперь автоматически создает интерфейс `opkgtun0` (L3) или `opkgtap0` (L2) и регистрирует его в системном демоне KeeneticOS NDM как `OpkgTun0` / `OpkgTap0`.
  - **Видимость в системном веб-интерфейсе роутера**: Туннель отображается в родном веб-интерфейсе Keenetic, списках сетевых подключений и живых графиках трафика.
  - **Приоритеты подключений (Multi-WAN / Policy Routing)**: Благодаря установке `security-level public` и `ip global auto` интерфейс QuakeLive-VPN можно назначать в системные политики маршрутизации KeeneticOS для направления трафика выбранных устройств или доменных списков.
  - **L2 TAP режим с эмуляцией ARP**: Реализована встроенная поддержка `IFF_TAP` (`LinuxTap`) с автоматическими ответами на ARP-запросы ядра роутера и бесшовной инкапсуляцией Ethernet-фреймов в L3 UDP-пакеты QuakeLive-VPN.
  - **Выбор режима в веб-панели**: В веб-интерфейсе SmartVPN добавлен блок управления интерфейсом с переключением между `OpkgTun` (L3), `OpkgTap` (L2) и `qlvpn0` (стандартный Linux TUN без NDM).
  - **Корректная дерегистрация**: При остановке или отключении клиента конфигурация интерфейса в NDM безопасно удаляется (`no interface OpkgTun0`), исключая появление «мусорных» интерфейсов после перезагрузки.
  - **Интеграция с мониторингом родных VPN**: В списке Native VPN интерфейсы `OpkgTun` и `OpkgTap` распознаются с собственным стилизованным бейджем.

## [1.0.83] - 2026-09-06
### Исправлено и Улучшено
- **Android App — Отображение телеметрии и скролл дашборда**:
  - **Исправление отображения карточки метрик**: Включен вертикальный скролл экрана (`verticalScroll`), оптимизированы вертикальные отступы и размеры компонентов, благодаря чему нижняя карточка со скоростями скачивания, отдачи и пингом теперь гарантированно видна на экранах любых устройств.
  - **Числовые значения телеметрии**: Числа скорости (`0 KB/s`) и пинга (`0 мс`) четко отображаются как при активном подключении, так и в отключенном состоянии, устраняя эффект пустой плашки без данных.

## [1.0.82] - 2026-09-06
### Исправлено и Улучшено
- **Android App — Надежное управление подключением, защита от зависаний при смене сети и детальный мониторинг**:
  - **Устранение зависаний при переподключении и смене сети (Wi-Fi ↔ Мобильная сеть)**:
    - Реализована надежная отмена рукопожатия и туннеля в любой момент времени (`connectJob`, `activeHandshakeSocket.close()`). Теперь нажатие на кнопку или переключатель мгновенно прерывает подключение без зависаний.
    - В поток приема UDP RX добавлен `socket.soTimeout = 2500` и встроенное обнаружение мертвого пира (dead peer detection): если входящие пакеты отсутствуют более 15 секунд, сессия корректно завершается и инициируется авто-переподключение.
    - Улучшен `NetworkCallback`: при смене активного сетевого интерфейса (Wi-Fi ↔ LTE) туннель оперативно перезапускается с новым сетевым маршрутом.
  - **Исправление статуса первого подключения**:
    - Устранено появление надписи «Переподключение» при первом старте: разделены состояния первичного подключения (`isConnecting`) и автоматического реконнекта (`isReconnecting`).
  - **Строгие 3 статуса в верхнем бейдже**:
    - Верхний статус теперь принимает строго 3 канонических состояния: `Подключено`, `Отключено`, `Подключение...`.
  - **Детальный статус фаз подключения под главной кнопкой**:
    - В чипе под главной кнопкой отображаются подробные стадии: поиск адреса сервера, защита UDP сокета, номер попытки рукопожатия (`Согласование (1/4)...`), создание TUN адаптера, запуск шифрования, либо статус переподключения с таймером и номером попытки.

## [1.0.81] - 2026-09-05
### Добавлено и Улучшено
- **Android App — Релизная сборка и полный автоматический запрос разрешений**:
  - **Релизная конфигурация сборки**: В `build.gradle.kts` настроен подписанный релизный билд (`signingConfig = debug`) с чистым идентификатором пакета `com.snakelair.qlvpn` для официальной установки на любые Android устройства без ограничений отладки.
  - **Автоматический запрос разрешений при старте**: При первом запуске приложение последовательно запрашивает все необходимые системные разрешения:
    - `POST_NOTIFICATIONS` (Android 13+ / API 33+) для фоновой службы туннеля и отображения скорости в шторке уведомлений.
    - `CAMERA` для мгновенного сканирования QR-кодов с токенами серверов.
    - Исключение из оптимизации батареи (`REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`) для предотвращения остановки VPN системой в спящем режиме.
  - **Защита в сканере QR-кода**: В экран сканирования QR добавлен информативный UI с кнопкой «Разрешить камеру» при отсутствии системного разрешения.

## [1.0.80] - 2026-09-05
### Исправлено
- **Android App & Tooling — Исправление кодировки скрипта установки `install_apk.bat`**:
  - **Устранение UTF-8 BOM**: Удален байт порядка следования (BOM `\xEF\xBB\xBF`) из заголовка `install_apk.bat`, из-за которого командная оболочка Windows `cmd.exe` сбоила с ошибкой `'я╗┐@chcp' не является внутренней или внешней командой` и выводила кракозябры.
  - **Строгие CRLF-окончания строк**: Восстановлены канонические Windows CRLF-переносы строк (`\r\n`), гарантирующие корректный парсинг команд командным процессором CMD.
  - **Удобный корневой скрипт**: В корень репозитория добавлен проксирующий скрипт `install_apk.bat` для быстрого запуска установки прямо из корневой директории проекта.
  - **Флаг `-d` (Allow Downgrade)**: В команду `adb install` добавлен параметр `-d` для предотвращения ошибок перезаписи при установке сборок.

## [1.0.79] - 2026-09-05
### Исправлено и Улучшено
- **Android App — Раздельное туннелирование: Асинхронная подгрузка приложений без зависаний UI и исправление кнопки «Сбросить»**:
  - **Асинхронная загрузка и кэширование (`InstalledAppsRepository`)**: Сканирование установленных пакетов системы (`pm.getInstalledApplications`, `getLaunchIntentForPackage`, `getApplicationLabel`) перенесено из главного UI-потока в фоновый пул корутин (`Dispatchers.IO`). При открытии приложения запускается фоновая предзагрузка, благодаря чему переключение на вкладку раздельного туннелирования происходит мгновенно (0 мс) без задержек и фризов.
  - **Плавный индикатор загрузки**: При первом сканировании на экране отображается анимированный спиннер с пояснением «Загрузка списка приложений... • Асинхронное сканирование пакетов системы», а при фоновом обновлении списка — аккуратная тонкая полоса прогресса `LinearProgressIndicator`.
  - **Исправление кнопки «Сбросить»**: Устранена проблема вертикального переноса букв («С\nб\nр\nо\nс»). Заголовок получил эластичную ширину `Modifier.weight(1f)`, а сама кнопка оформлена в виде горизонтального бейджа с иконкой `DeleteSweep`, явными отступами и жестким запретом вертикального переноса (`softWrap = false, maxLines = 1`).
  - **Кнопка «Обновить список»**: В заголовок экрана добавлена иконка обновления (`Refresh`) для повторного сканирования пакетов при установке или удалении приложений в системе без перезапуска VPN.
  - **Быстрое действие «Снять все»**: В заголовок секции «ВЫБРАННЫЕ ДЛЯ ОБХОДА» добавлена прямая ссылка «Снять все» для мгновенного сброса отмеченных приложений в один клик.

## [1.0.78] - 2026-09-05
### Добавлено и Улучшено
- **Android App — Прямой встроенный SSH-клиент для установки сервера на VPS без сторонних утилит**:
  - **Встроенная поддержка SSH (JSch Pure Java)**: В мобильное приложение интегрирована библиотека `com.github.mwiede:jsch` для прямого безопасного SSH-соединения с сервером VPS прямо со смартфона без необходимости использования внешних терминалов (Termius, JuiceSSH) или роутера.
  - **Утилита `VpsSshDeployer`**: Фоновый движок на Kotlin Coroutines (`Dispatchers.IO`) с поддержкой аутентификации по паролю, очистки управляющих кодов ANSI, потоковой передачи консольного вывода в реальном времени и интеллектуального поиска выданного сервером токена `qlvpn://`.
  - **Интерактивный терминал установки**: Модальное окно «Установка на VPS» дополнено кнопкой «🚀 Установить напрямую с телефона (SSH)» и живым терминалом вывода логов установки с отображением статуса в реальном времени.
  - **Автоматический импорт токена в 1 клик**: По завершении установки приложение автоматически извлекает токен `qlvpn://...`, сохраняет его в список подключений и предлагает мгновенно активировать туннель («⚡ Подключиться сейчас»).

## [1.0.77] - 2026-09-05
### Добавлено и Улучшено
- **Android App — Раздельное туннелирование: Полная видимость всех приложений (`QUERY_ALL_PACKAGES`) и приоритет выбранных**:
  - **Разрешение `QUERY_ALL_PACKAGES`**: В `AndroidManifest.xml` добавлено системное разрешение `QUERY_ALL_PACKAGES` с подавлением предупреждения, устраняющее фильтрацию видимости пакетов Android 11+ (API 30+). Теперь в приложении видны абсолютно все установленные программы.
  - **Отображение и фильтрация приложений**: В список включены все пользовательские и обновленные приложения, приложения с лаунчером, приложения с сетевыми разрешениями, а также добавлен переключатель-чип «Системные» для отображения внутренних системных сервисов ОС при необходимости.
  - **Перемещение выбранных приложений наверх списка**: Выбранные для обхода приложения динамически перемещаются в верхнюю часть списка в выделенную секцию «ВЫБРАННЫЕ ДЛЯ ОБХОДА (N)» с акцентной рамкой и чекбоксом. При снятии отметки приложение плавно возвращается в нижнюю секцию «ОСТАЛЬНЫЕ ПРИЛОЖЕНИЯ».
  - **Быстрые фильтр-чипы**: Добавлены фильтр-чипы «Все», «Выбранные (N)» и «Системные» для мгновенной навигации по установленному ПО.
- **Android App & Core — Полнофункциональная настройка и развертывание на VPS (как в главном веб-приложении)**:
  - **Двухвкладочное модальное окно VPS**: В окне «Установка на VPS» добавлены вкладки «Параметры VPS» и «Быстрый старт».
  - **Полный набор параметров VPS**: Настройка IP/хоста VPS, SSH-порта (22), пользователя (root), пароля root (с кнопкой показа/скрытия 👁), UDP-порта сервера (27960), HTTPS-порта веб-админки (8092) и пароля веб-админки (с кнопкой 🎲 «Случайный» генератор `Pass-xxxxxx`).
  - **Динамическая генерация команд**: Автоматическое формирование готовой команды для bash-терминала VPS с переменными `SERVER_PORT`, `WEB_PORT`, `WEB_PASS` и готовой строки подключения `ssh -p ... user@host` с кнопками копирования в 1 клик.
  - **Автоматическая установка через Keenetic роутер**: Возможность отправить задачу развертывания на SmartVPN API роутера в 1 клик прямо из мобильного приложения. Приложение опрашивает статус, выводит живой лог терминала установки и при успехе автоматически добавляет сервер в список и предлагает немедленно подключиться.
  - **Динамические скрипты `install_vps.sh`**: Скрипты установки сервера на VPS обновлены для поддержки переменных окружения `SERVER_PORT`, `WEB_PORT`, `WEB_PASS` в UFW, firewalld, systemd сервисе и генерации начального токена.

## [1.0.76] - 2026-09-05
### Улучшено и Исправлено
- **Android App — Главная эмблема кнопки «В БОЙ» на прозрачном фоне и чистый UI**:
  - **Интеграция мастер-эмблемы на прозрачном фоне**: Исходное высококачественное изображение `assets/image_transp.png` центрировано, очищено от пустых полей и конвертировано в нативный прозрачный ассет `quake_emblem.png` (720x720) с субпиксельным сглаживанием Эрмита и чистым альфа-каналом.
  - **Устранение круглой рамки (Stroke Removal)**: Полностью удалена круглая рамка (`Stroke(width = 5.dp)`) в активном состоянии («В бою»). Кнопка избавлена от жестких границ и круговой обводки, сохраняя плавное рассеянное свечение и динамическую нейросеть на фоне.
  - **Удаление наложенного текста «В бой» / «В бою»**: Полностью убран поверхностный баннер-плашка с текстами «В БОЙ» и «В БОЮ», перекрывавший нижние крылья и лезвия эмблемы. Эмблема отображается полностью открытой и чистой.
  - **Информативный чип статуса**: Статус («В БОЮ • 100% DPI BYPASS» / «В БОЙ • НАЖМИТЕ ДЛЯ ПОДКЛЮЧЕНИЯ») вынесен в отдельный элегантный чип прямо под эмблемой с индикатором режима.
  - **Металлический шиммер без искажения контуров**: Эффект переливающегося блеска в бою переведен на композитинг `BlendMode.SrcAtop` в оффскрин-слое, благодаря чему блик омывает только видимые непрозрачные пиксели эмблемы и не выходит за ее пределы.

## [1.0.75] - 2026-09-05
### Добавлено и Улучшено
- **Android App — Режимы раздельного туннелирования (Split Tunneling)**:
  - **Выбор режима «Только выбранные через ВПН» (Route Only)**: Добавлен явный режим маршрутизации (`builder.addAllowedApplication`), при котором через QuakeLive-VPN идут исключительно отмеченные пользователем приложения (например, YouTube, браузер, Discord), а весь остальной трафик устройства направляется напрямую через провайдера/Wi-Fi.
  - **Режим «Исключить выбранные из ВПН» (Bypass)**: Сохранен режим полного VPN с исключением указанных приложений (`builder.addDisallowedApplication`, например банковских клиентов и Госуслуг).
  - **Наглядные карточки выбора**: Вместо одиночного переключателя интерфейс экрана «Обход» оснащен двумя выразительными карточками с индивидуальными иконками, акцентной подсветкой и подсказками.
  - **Кнопка быстрой очистки**: Добавлена кнопка мгновенного сброса выбранных приложений.
- **Android App — Языконезависимый и регистронезависимый поиск приложений (`AppSearchHelper`)**:
  - **Поиск по ключевым словам и брендам**: Мгновенный поиск приложений YouTube при вводе «ю», «ют», «ютуб»; Google Chrome при вводе «хром»; Telegram при вводе «телега» / «тг»; Discord при вводе «дискорд» / «дс»; СберБанк при вводе «сбер»; и многих других сервисов.
  - **Двунаправленная фонетическая транслитерация (русский <-> латиница)**: Специализированная обработка символов («ю» -> `yu`, `u`, `you`; «х» -> `ch`, `kh`, `h`; «ж», «ч», «ш», «щ»).
  - **Автоматическое исправление раскладки клавиатуры**: Поддержка ошибочного ввода на русской раскладке (например, «нщге» автоматически находит YouTube, «сркщь» — Chrome).
  - **Отображение системных приложений**: В список включены все пользовательские и системные приложения с активным лаунчером (включая предустановленный YouTube и браузеры).
- **Развертывание QuakeLive-VPN на VPS (Linux VPS Deployment)**:
  - **Автоматический скрипт установки `install_vps.sh`**: Создан скрипт автоматической установки для Ubuntu 20.04+, Debian 11+, AlmaLinux, Rocky Linux с авто-определением архитектуры (`x86_64`, `aarch64`), настройкой ядра (`net.ipv4.ip_forward=1`, BBR), правил iptables NAT (MASQUERADE) и системного демона `systemd`.
  - **Генерация токенов и ANSI QR-код в консоли**: Скрипт сразу создает первый токен доступа и выводит в терминал `qlvpn://` ключ и QR-код для мгновенного сканирования камерой.
  - **Консольная утилита управления `qlvpn-admin`**: Быстрое добавление (`add`), просмотр (`list`), отзыв токенов (`del`) и просмотр логов (`logs`).
  - **Интерактивное руководство в Android App**: В экраны «Серверы» и «Настройки» добавлено модальное окно «Установка на VPS» с пошаговой инструкцией и копированием команды в 1 клик: `curl -sSL https://raw.githubusercontent.com/snakelair/SmartVpn/master/install_vps.sh | bash`.
  - **Подробная документация `VPS_INSTALL.md`**: Пошаговое руководство с таблицей команд, требованиями к портам (27960/UDP, 8092/TCP) и описанием веб-панели управления.

## [1.0.74] - 2026-09-05
### Улучшено и Исправлено
- **Android App — Чистый прозрачный фон и векторная четкость эмблемы «В БОЙ»**:
  - **Устранение квадратной рамки**: Переработан рендеринг кнопки подключения в `BattleEmblemButton`. Контейнер кнопки жестко ограничен круглой формой `Modifier.clip(CircleShape)` — ликвидирован прямоугольный ripple-эффект нажатия и любые фоновые границы.
  - **Круговой металлический блик**: Прямоугольная отрисовка `drawRect` с аппаратным режимом наложения `BlendMode.Plus` заменена на `drawCircle` со сферическим радиусом, благодаря чему переливающийся шиммер в бою омывает исключительно окружность медальона без артефактов квадратных углов.
  - **Безупречная прозрачность эмблемы (Pure Alpha Antialiasing)**: Сформировано высокодетализированное изображение эмблемы Quake Live (720x720) с субпиксельным сглаживанием Эрмита по хромированному внешнему кольцу и 100% прозрачным фоном (A=0).
  - **Нативный векторный ассет (VectorDrawable)**: В проект добавлен масштабируемый вектор `ic_quake_emblem_vector.xml` с градиентами рубинового купола, хромированными лезвиями и фасеточным острием без растровых пикселей и фона.

## [1.0.73] - 2026-09-05
### Исправлено и Оптимизировано
- **Полное устранение нагрузки CPU в простое на роутере Keenetic (0.0% CPU Idle Fix)**:
  - **Устранение зависания `ndmc -c show running-config`**: Выявлена и устранена причина перегрузки CPU (~30% smart-vpn и ~30% ndm) — неинтерактивный вызов `show running-config` в Keenetic OS блокировался терминальным пейджером без TTY и висел до 8-секундного таймаута, создавая бесконечный шторм процессов. Вызов исключен из регулярного опроса, чтение конфига переведено на RAM-диск `/tmp/run/running-config` с 10-минутным кэшированием.
  - **Адаптивный фоновый опрос интерфейсов**: Внедрен интеллектуальный двухрежимный опрос (3 сек при активном Web UI / WebSocket и 30 сек в режиме простоя), снизивший нагрузку на процессор роутера в простое до 0.0%. При открытии Web UI опрос активируется немедленно.
  - **Оптимизация таймеров и защита от спин-локов**: В QuakeLive-VPN таймер `gameClientSnapshotLoop` переведен с 200 мс на 1 сек, в циклы чтения TUN и сокетов добавлены защитные паузы при ошибках ввода-вывода. Таймер CPU Watchdog в Sing-Box оптимизирован.
- **Android App — Анимированная нейросеть и переливающаяся эмблема «В БОЙ»**:
  - **Единая иконка-эмблема кнопки «В БОЙ»**: Центральная кнопка подключения оформлена как полноразмерная монолитная фирменная эмблема Quake Live (170dp) из мастер-ассета.
  - **Переливающийся перламутровый шиммер (Iridescent Glow)**: В активном бою кнопка непрерывно переливается динамическим вращающимся спектральным градиентом (Emerald, Cyan, Mint, Lime, Electric Blue) и диагональным металлическим бликом.
  - **Анимированная нейросеть с бегущими данными (Neural Network)**: На фоне кнопки развернут динамический холст нейросети с узлами (nodes), синаптическими связями и потоками светящихся пакетов данных, передающими статус активного VPN-трафика в реальном времени.

## [1.0.72] - 2026-09-05
### Добавлено и Улучшено
- **Полное обновление графики и фирменного стиля QuakeLive-VPN**:
  - **Android Launcher & Adaptive Icons**: Внедрен официальный комплект лаунчер-иконок для всех разрешений экрана (`mdpi` 48px, `hdpi` 72px, `xhdpi` 96px, `xxhdpi` 144px, `xxxhdpi` 192px) со стандартными квадратными и круглыми иконками (`ic_launcher_round.png`).
  - **Адаптивные иконки Android 8.0+ (API 26+)**: Добавлены слои фона (`ic_launcher_background.png`) и светящейся эмблемы (`ic_launcher_foreground.png`) в `res/mipmap-anydpi-v26/`.
  - **Эмблема в интерфейсе Android (Compose UI)**:
    - В верхний заголовок `DashboardScreen` добавлена светящаяся эмблема Quake Live с мечом.
    - В главную кнопку подключения («В БОЙ» / «ОТКЛЮЧИТЬ») интегрирована центральная эмблема.
    - В заголовок и карточку «О приложении» экрана `SettingsScreen` добавлена фирменная эмблема.
    - В шторку системных уведомлений Android добавлена цветная эмблема высокого разрешения (`setLargeIcon`).
  - **Web UI & Standalone Control Center (`:8091` и `:8092`)**:
    - Обновлен файл `quake_logo.png` до четкой эмблемы высокого разрешения (512x512).
    - Обновлены фавиконы `favicon.ico`, `favicon-32x32.png`, `favicon-16x16.png`, `android-chrome-192x192.png`.
    - Добавлен широкоформатный промо-баннер `quake_banner.png`.
    - Улучшена стилизация логотипа в навигационной панели и заголовке (скругление 6px, мягкое свечение).
  - **Windows Tray**: Интегрирована контрастная и четкая трей-иконка `quake_tray_32.png`.

## [1.0.71] - 2026-09-05
### Добавлено и Улучшено
- **Автоматическое переподключение в Android-клиенте (Auto-reconnect)**:
  - **Настройка в приложении**: В панель навигации добавлен экран «Настройки» с переключателем «Автоматическое переподключение» (включено по умолчанию), автозапуском при перезагрузке и информацией о протоколе.
  - **Интеллектуальный цикл повторных попыток**: При разрыве соединения (смена сети Wi-Fi ↔ LTE, потеря keepalive, таймаут сокета) служба VPN автоматически планирует переподключение с экспоненциальной задержкой без закрытия foreground-сервиса и сброса интерфейса.
  - **Интеграция с Android NetworkCallback**: Мгновенный перехват событий смены или восстановления интернет-соединения через `ConnectivityManager.NetworkCallback` для немедленного переподключения.
  - **Визуальная индикация переподключения**: Статусные бейджи и чипы в приложении и уведомлении отображают `«ПЕРЕПОДКЛЮЧЕНИЕ...»` янтарным цветом с анимацией повтора.
  - При явном отключении пользователем (`«ОТКЛЮЧИТЬ»`) авто-переподключение отменяется.

## [1.0.70] - 2026-09-05
### Улучшено и Исправлено
- **Оптимизация UI статусов в Android-клиенте QuakeLive-VPN**:
  - **Разделение бэйджа статуса**: Устранено нежелательное разбиение строки статуса на 2 строки на компактных экранах. В верхнем статусном бэйдже оставлен лаконичный статус подключения (`«В БОЮ»` / `«ПОДКЛЮЧЕНИЕ...»` / `«ОТКЛЮЧЕНО»`).
  - **Выделенный чип обхода DPI**: Под главной кнопкой подключения добавлен стильный акцентный чип со статусом маскировки и обхода ТСПУ (`«100% DPI BYPASS • ТСПУ НЕВИДИМ»` в активном состоянии и `«МАСКИРОВКА ПОД QUAKE LIVE»` в режиме ожидания).
  - **Индикатор в карточке сервера**: В заголовке карточки активного сервера добавлен значок подтверждения активного обхода DPI при установленном соединении.
- **Увеличение времени ожидания и попыток рукопожатия (Устранение ошибки Poll timed out)**:
  - Увеличено количество попыток рукопожатия с 3 до 6 на Android с прогрессивным увеличением таймаута (1.5с -> 2.0с -> 2.5с -> 3.0с -> 3.5с -> 4.0с) и экспоненциальной паузой между повторами (общее время ожидания до 18 секунд).
  - В случае задержки в мобильной сети больше не выбрасывается сырая системная ошибка `Poll timed out`, а отображается понятный пользователю статус.
  - В Go-клиенте количество попыток рукопожатия увеличено до 5.

## [1.0.69] - 2026-09-05
### Исправлено и Улучшено
- **Устранение зависания сетевого трафика и счетчиков в Android-клиенте**:
  - **Исправление мутации длины буфера DatagramPacket**: В Java `DatagramSocket.receive(packet)` автоматически перезаписывает `packet.length` фактическим размером принятого UDP-пакета (например, 59 байт keepalive). Все последующие приемы пакетов данных обрезались до 59 байт, что ломало дешифрование AES-GCM. Добавлен обязательный сброс `recvPacket.length = recvBuf.size` перед каждым вызовом `receive()`.
  - **Предотвращение сброса сессии при неблокирующем чтении из TUN**: В `QuakeVpnService` включен режим `builder.setBlocking(true)`. Устранено преждевременное закрытие сессии в потоке `txThread` при возврате `read <= 0`.
  - **Динамическое выделение IP и очистка устаревших сессий на сервере**: В `internal/qlvpn/server.go` добавлена автоматическая очистка устаревших сессий по имени игрока или IP-адресу при повторном рукопожатии, а также динамическое выделение свободного адреса из пула `10.80.0.2..254`.
  - **Порядок установления туннеля на Android**: Рукопожатие `initHandshake` вынесено до вызова `builder.establish()`, что позволяет конфигурировать сетевой интерфейс TUN Android точным IP-адресом, выделенным сервером.
  - Добавлено подробное протоколирование сетевых событий с тегом `QLVPN` в Android Logcat.

## [1.0.68] - 2026-09-05
### Добавлено и Улучшено
- **Запрет повторного запуска Windows-клиента (Single Instance Enforcement)**:
  - Добавлен системный мьютекс `Local\QuakeLiveVPN_GUI_Mutex` для защиты от параллельного запуска нескольких экземпляров `ql-vpn.exe`.
  - Проверка выполняется до UAC-элевации прав: при повторном запуске приложение не требует подтверждения администратора, посылает вызов `/api/show` существующему процессу, фокусирует окно конфигуратора в браузере и мгновенно завершается (`os.Exit(0)`).
  - В Web UI добавлен эндпоинт `/api/show` (`/api/qlvpn/show`), открывающий/фокусирующий окно конфигуратора в режиме приложения (`--app`).
- **Полноценный сетевой движок Android (Двустороннее туннелирование трафика)**:
  - Реализован сетевой движок на Kotlin: устранена проблема отсутствия сетевого трафика в Android-приложении.
  - 1-RTT Quake Live рукопожатие: генерация ключей X25519 (лестница Монтгомери RFC 7748), проверка HMAC-SHA256 подписи сервера и токена.
  - Аппаратное шифрование/дешифрование AES-128-GCM со 128-битным тегом через аппаратный криптоускоритель Android (ARM-CE).
  - Защита физического сокета через `VpnService.protect(socket)` от зацикливания маршрутизации.
  - Двусторонние циклы чтения/записи IP-пакетов между `ParcelFileDescriptor` TUN и UDP сокетом сервера.
  - Автоматический Keepalive каждые 5 секунд при простое соединения и расчет реальной скорости RX/TX и сетевой задержки.

## [1.0.67] - 2026-09-05

### Добавлено и Улучшено
- **Сборка, оптимизация и развертывание QuakeLive-VPN для Android на физическое устройство**:
  - Полная адаптация и компиляция Android APK на базе Jetpack Compose Material 3 и Android 16 / VanillaIceCream (`compileSdk 35`, Java 21, Gradle 8.11.1).
  - Нативная сборка и успешная установка APK `QuakeLiveVPN.apk` на подключенный смартфон POCO / Xiaomi (`754a7ea9`).
  - Улучшено отображение карточки сервера на главном экране (Dashboard): имя игрока (`🎮 Ranger`), адрес сервера (`194.58.33.191:27960`) и назначенный IP отображаются сразу после импорта профиля по deep-link ссылке `qlvpn://`.
  - Заменены устаревшие разделители на `HorizontalDivider` и `VerticalDivider` для совместимости со свежими библиотеками Jetpack Compose M3.
  - Добавлены скрипты `android/build_apk.bat` и `android/install_apk.bat` для сборки и установки в один клик.

## [1.0.66] - 2026-09-05
### Добавлено и Улучшено
- **Реализация Android-клиента и мобильного Go-моста (QuakeLive-VPN for Android)**:
  - Создан пакет `mobile/qlvpnandroid` для компиляции Go-ядра в библиотеку Android AAR (`gomobile bind`).
  - В ядро `Client` добавлена поддержка интерфейса `SocketProtector` (`VpnService.protect(socketFd)`), предотвращающего петли маршрутизации Android при перенаправлении `0.0.0.0/0`.
  - Добавлен адаптер `AndroidTunDevice` в `internal/qlvpn/tun.go` для прямой работы с файловым дескриптором Android `VpnService.Builder`.
  - Реализован полный проект нативного Android-приложения на **Jetpack Compose (Material 3)**:
    - **Dashboard**: анимированная кнопка подключения, живой расчет скорости RX/TX, замер сетевой задержки пинга и статус 100% Anti-DPI.
    - **Менеджер серверов**: встроенный сканер QR-кодов (ML Kit + CameraX) и поддержка ссылок `qlvpn://`.
    - **Раздельное туннелирование (Split Tunneling)**: фильтр приложений для обхода VPN (банки, Госуслуги) или туннелирования только выбранных.
    - **Quick Settings Tile (`TileService`)**: системная плитка в шторке Android для управления туннелем в один клик.
  - Подготовлены кросс-платформенные скрипты сборки библиотеки AAR: `build_aar.bat` и `build_aar.sh`.
