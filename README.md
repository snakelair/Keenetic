...
### 6. 📹 `smart-nvr` (v1.0.0)
**Высокопроизводительный шлюз видеонаблюдения и веб-центр для регистраторов Hiseeu / Xiongmai и камер iCSee:**
- **Проксирование живого видео и архива:** безопасный доступ к камерам и NVR из внешней сети через интернет (KeenDNS, VPN, WAN) без прямого проброса портов.
- **0% перекодирования (Zero Transcoding):** процессор роутера не нагружается декодированием — аппаратное воспроизведение H.264/H.265 выполняется на клиенте.
- **Автопоиск камер в сети (Network Discovery):** сканирование домашней сети за 1.5 сек с определением моделей, серийных номеров и добавлением в 1 клик.
- **Интерактивный архив (Playback):** 24-часовая временная шкала с полосами записи, встроенный плеер и прямое скачивание видеоотрезков.
- **PTZ-управление:** поворот, наклон и зум для поворотных камер прямо из браузера.
- **Сверхнизкое потребление ресурсов:** всего 12–18 МБ RAM, логирование в RAM (`/tmp/smart-nvr.log`).
- **Веб-интерфейс:** `http://172.16.1.1:8095` (или IP вашего роутера)
- [📘 **Руководство пользователя SmartNVR (smart_nvr_user_guide.md)**](smart_nvr_user_guide.md)
...
```bash
# Установить SmartNVR (Шлюз видеонаблюдения, Поиск камер, Архив и Live):
